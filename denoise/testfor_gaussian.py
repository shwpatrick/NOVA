...
parser = argparse.ArgumentParser(description="GaussianFilter_Test")
...
def main():
    ...
    output_dir = f"demo_gaussian{opt.kernel_size}x{opt.kernel_size}"
    ...
    for idx, f in enumerate(files_source, start=1):
        ...
        denoised = cv2.GaussianBlur(noisy_img, (opt.kernel_size, opt.kernel_size), opt.sigma)
        ...
        titled = add_titles(stacked, ['Original', 'Noisy', 'Gaussian'])
