...
import glob
import numpy as np
from utils import *  # 為了使用 batch_PSNR

parser = argparse.ArgumentParser(description="MeanFilter_Test")
parser.add_argument("--test_data", type=str, default='Set12', help='test on Set12 or Set68')
parser.add_argument("--test_noiseL", type=float, default=25, help='noise level used on test set')
parser.add_argument("--kernel_size", type=int, default=3, help='mean filter kernel size')
...
def main():
    ...
    output_dir = f"demo_mean{opt.kernel_size}x{opt.kernel_size}"
    ...
    for idx, f in enumerate(files_source, start=1):
        ...
        denoised = cv2.blur(noisy_img, (opt.kernel_size, opt.kernel_size))
        ...
        # 計算 PSNR
        psnr = batch_PSNR(torch.Tensor(denoised).unsqueeze(0).unsqueeze(0),
                          torch.Tensor(Img_norm).unsqueeze(0).unsqueeze(0), 1.)
        psnr_total += psnr

        # 存圖
        stacked = np.hstack((clean_img, noisy_img_, denoised_img))
        titled = add_titles(stacked, ['Original', 'Noisy', 'MeanFilter'])
